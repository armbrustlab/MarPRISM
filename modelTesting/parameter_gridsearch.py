...
def xgboost_search(data_file, label_file, out):
    ...
    xg = XGBClassifier()
    grid = GridSearchCV(xg, param_grid=param_grid, n_jobs=-1, cv=5, verbose=3, scoring='f1_weighted')
...
def randomforest_gridsearch(data_file, label_file, out):
    ...
    grid = GridSearchCV(rf, param_grid=param_grid, n_jobs=-1, cv=5, verbose=3, scoring='f1_weighted')
...
def nn_gridsearch(data_file, label_file, out):
    ...
    grid = GridSearchCV(model, param_grid=param_grid, cv=5, verbose=5, scoring='f1_weighted')
